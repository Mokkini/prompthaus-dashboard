...
import { AlertCircle, ExternalLink, Pencil, PlusCircle } from "lucide-react";

export default async function AdminPromptsPage() {
  // 1. Supabase User-Client und User holen
  ...
  // 3. Supabase Admin-Client und Prompt-Pakete laden
    ...
    return (
      <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Fehler beim Laden der Prompt-Pakete: {error.message}
          </AlertDescription>
        </Alert>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Willkommen im Admin-Bereich, {user.email}.
          </p>
        </div>

        {/* Button‑Gruppe: Zur Hauptseite + Neues Paket */}
      ...
      <div className="space-y-4">
        ...
        {prompts && prompts.length > 0 ? (
          ...
          <div className="text-center py-10 border border-dashed rounded-md">
            <p className="text-muted-foreground">
              Noch keine Prompt-Pakete in der Datenbank gefunden.
            </p>
          </div>
        )}
      </div>
